...
def parse_args():
    ...
    parser.add_argument(
        "--eval", type=str, choices=["best", "best_20", "final"], default="best"
    )
...
def main():
    ...
    for fold in range(5):
        ...
        # 1500 Samples from 5 folds. Change this to get a full evaluation.
        ...
        if args.eval == "best":
            # print(args.model_dir, f'{args.kernel_type}_best_fold{fold}')
        ...
        this_LOGITS, this_PROBS = val_epoch(
            model,
            valid_loader,
            mel_idx,
    ...
    dfs2 = dfs.copy()
    for i in range(5):
        dfs2.loc[dfs2["fold"] == i, "pred"] = dfs2.loc[dfs2["fold"] == i, "pred"].rank(
            pct=True
        )
    ...
    for i in range(5):
        dfs3.loc[dfs3["fold"] == i, "pred"] = dfs3.loc[dfs3["fold"] == i, "pred"].rank(
            pct=True
        )
    auc_20_rank = roc_auc_score(dfs3["target"] == mel_idx, dfs3["pred"])

    np.save(
        "../../results/CNN-Ha/FalsePos_" + args.kernel_type, fpr,
    )
    np.save(
        "../../results/CNN-Ha/TruePos_" + args.kernel_type, tpr,
    )
    np.save(
        "../../results/CNN-Ha/Threshold_" + args.kernel_type, thr,
    )
    ...
    with open(os.path.join("../../results/README_CNN-Ha.md"), "a+",) as appender:
    ...
    plt.legend(loc="best")
    plt.show()
    plt.savefig("../../results/CNN-Ha/roc_curve" + args.kernel_type + ".png", dpi=300)
